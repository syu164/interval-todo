...
function add(todo) {
  var todoText = input.value; // localStrage に先に値を持っていたら入れる
  ...
  if (todoText) {
    // if (todoText.length > 0) の条件だとCannot Read Propatyになる
    var li = document.createElement("li");
    li.innerText = todoText;
    li.classList.add("list-group-item"); // 完了の状態の打ち消し線を保っておく処理
...
var interval;
var mainButton = document.getElementById('js-btn');
...
function getRemainingTime(endTime) {
  ...
  var elapsedTime = timer.wholeTime - total;
  return {
...
function updateClock() {
  ...
  var minutes = "".concat(remainingTime.minutes).padStart(2, '0');
  var seconds = "".concat(remainingTime.seconds).padStart(2, '0');
...
function switchMode(mode) {
  ...
  document.querySelector("[data-mode=\"".concat(mode, "\"]")).classList.add('active');
  document.body.style.backgroundColor = "var(--".concat(mode, ")"); // TODO タイマーの色を変更するコードに書き換える
...
function init() {
  // タイマー用のキャンバス
  var timerCanvas = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#timerFace')[0];
...
function drawTimer(ctxTimer, remainingTime) {
  ...
  var x = intervalTodoObj.timerCanvasWidth / 2;
  var y = intervalTodoObj.timerCanvasHeight / 2;
  var r = intervalTodoObj.timerCanvasWidth * 1.5 / 2; // 対角線の長さの半分

  var percent = remainingTime.elapsedTime / timer.wholeTime;
  var drawDeg = percent * 360;
  var endDeg = getRadian(drawDeg);
  var timerFaceColor = 'rgba(0, 220, 0, 0.5)';
